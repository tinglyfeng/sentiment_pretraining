...
class upCatBranch(nn.Module):
    def __init__(self, backbone):
        ...
        if backbone == 'resnet18':
            ...
            self.up1 = convUpBlock(2, 128, 128)
            self.up2 = convUpBlock(4, 256, 128)
            self.up3 = convUpBlock(8, 512, 128)
            self.up4 = convUpBlock(16, 512, 128)
...
class colorHead(nn.Module):
    def __init__(self,backbone) :
        ...
        elif backbone == 'vgg16_bn' or backbone == 'vgg19_bn':
            self.conv = nn.Conv2d(512, 2, kernel_size=3, bias = True, padding = 3 //2)
...
class srHead(nn.Module):
    def __init__(self, backbone):
        ...
        if backbone == 'resnet18':
            ...
            self.deconv = nn.Sequential(
                nn.ConvTranspose2d(512, 512, kernel_size=3, stride=2, padding=3 // 2, output_padding=1),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(512, 512, kernel_size=3, stride=2, padding=3 // 2, output_padding=1),
                nn.ReLU(inplace=True)
            )
            self.reconstruction =nn.Sequential(
                nn.Conv2d(512, 3, kernel_size=3, padding=3 // 2), 
                nn.Sigmoid()
            )
...
class multiHeadVgg(nn.Module):
    ...
    def build_model(self):
        vgg_tv = models.__dict__[self.backbone](pretrained= self.is_pretrain)
...
if __name__ == '__main__':
    ...
    model = multiHeadVgg(None, None, None)
